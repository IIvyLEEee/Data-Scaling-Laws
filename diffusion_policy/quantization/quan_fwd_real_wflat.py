...
def conv1d_int8_fwd(x_int8, weight_int8, kernel_size=3, stride=1, padding=0, \
                    bias=bias, dilation=1, groups=1):
    ...
    padding = padding[0]
    stride = stride[0]
    kernel_size = weight_int8.shape[1] / x_int8.shape[1]
    
    if x_int8.dtype() != torch.int8: # W8A32 but calculate all in float32
        weight_bf16 = weight_int8.to(torch.bfloat16)
        out_int32 = F.conv1d(x_int8, weight_bf16, bias=None, stride=stride, padding=padding, dilation=dilation, groups=groups)
...
def linear_int8_fwd(x_int8, weight_int8, bias=None):
    ...
    if x_int8.dtype() != torch.int8:
        weight_int8 = weight_int8.to(torch.bfloat16)
        out_int32 = F.linear(x_int8, weight_int8, bias=None)
    else:
