...
import logging
from diffusion_policy.quantization.quan_layer import QuantModule, StraightThrough, lp_loss
from diffusion_policy.quantization.quan_model import QuantModel
from diffusion_policy.quantization.block_recon import LinearTempDecay
from diffusion_policy.quantization.adaption import AdaRoundQuantizer
from diffusion_policy.quantization.utils import save_inp_oup_data

logger = logging.getLogger(__name__)


def layer_reconstruction(model: QuantModel, layer: QuantModule, cali_data: torch.Tensor,
                         batch_size: int = 1, iters: int = 20000, weight: float = 0.001, opt_mode: str = 'mse',
    ...
    if not act_quant:
        layer.weight_quantizer = AdaRoundQuantizer(uaq=layer.weight_quantizer,weight_tensor=layer.org_weight.data,
