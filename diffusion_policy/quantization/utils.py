...
from diffusion_policy.quantization.quan_layer import QuantModule, Weight_Quantizer, Activation_Quantizer
...
def save_inp_oup_data(model: QuantModel, layer: Union[QuantModule, BaseQuantBlock], cali_data: torch.Tensor,
                      asym: bool = False, act_quant: bool = False, batch_size: int = 1, keep_gpu: bool = True,
    ...
    if not cond:
        ...
        cali_xs, cali_ts, cali_lconds, cali_conds = cali_data
    ...
    for i in trange(num):
        if not cond:
            ...
            cur_inp, cur_out = get_inp_out(
                cali_xs[i].to(device), 
                cali_ts[i].to(device),
                cali_lconds[i].to(device),
                cali_conds[i].to(device)
            ) if not is_sm else get_inp_out(
                cali_xs[inds[i]].to(device), 
                cali_ts[inds[i]].to(device),
                cali_lconds[inds[i]].to(device),
                cali_conds[inds[i]].to(device)
            )
...
def resume_cali_model(qnn: QuantModel, ckpt_path, cali_data, quant_act=False, act_quant_mode='qdiff', cond=False):
    ...
    if quant_act:       
        ...
        for m in qnn.model.modules():
            if isinstance(m, AdaRoundQuantizer):
                m.zero_point = nn.Parameter(m.zero_point)
                m.delta = nn.Parameter(m.delta)
            elif isinstance(m, (Activation_Quantizer, Weight_Quantizer)):
