...
class ConvTranspose1dFunc(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx, 
        input: torch.Tensor,
...
class QuantConvTranspose1d(nn.Module):
    def __init__(
        ...
        self.weight_delta = nn.Parameter(torch.full((in_channels,), 0.0), requires_grad=False)
    ...
    def forward(self, input: torch.Tensor):
        ...
        return ConvTranspose1dFunc.apply(
            input, 
            self.weight, 
            self.input_delta, 
            self.weight_delta, 
...
if __name__ == "__main__":
    ...
    # 创建标准 ConvTranspose1d 和自定义 QuantConvTranspose1d
    conv_ref = nn.ConvTranspose1d(in_channels, out_channels, kernel_size, 
                                 stride=stride, padding=padding, bias=False)
    quant_conv = QuantConvTranspose1d(in_channels, out_channels, kernel_size, 
                                     stride=stride, padding=padding, bias=False)
    ...
    # 输出结果
    print("dx (nn.ConvTranspose1d input grad):", x.grad)
    print("dy (QuantConvTranspose1d input grad):", y.grad)
