...
# 1. 原始输入 (模拟线性层的输入)
x_bf16_linear = torch.randn(6, 512).to(dtype=torch.bfloat16).cuda()  # [B, in_features]

# 2. 权重准备 (线性层的权重)
in_features = x_bf16_linear.shape[-1]
out_features = 768
weight_bf16_linear = torch.randn(out_features, in_features).to(torch.bfloat16).cuda()  # [out_features, in_features]
...
out_bf16_linear = out_fp32_linear.to(dtype=torch.bfloat16)
...
linear_layer = torch.nn.Linear(in_features, out_features).to(torch.bfloat16).cuda()
with torch.no_grad():
